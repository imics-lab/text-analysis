# -*- coding: utf-8 -*-
"""Runs k-fold CV on a SVC with tuned parameters. 

A classification report and confusion matrix are generated at the end.

Created on Thu Jul 23 12:01:14 2020
...
floattarget = list(np.nanmean(ratings, axis=1))
...
for train_i, test_i in KFold(n_splits=10).split(X_best):
    ...
    #create model
    svc = svm.SVC(C=10, class_weight='balanced')
...
prediction_df = pd.DataFrame(all_target, columns=['True label'])
prediction_df['Predicted label'] = all_preds
prediction_df.to_csv(save_csv_path)
